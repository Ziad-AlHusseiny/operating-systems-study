...
The lesson authoring record is a readable input to the canonical Lesson and Material Section records. Compilation keeps `id`, `moduleId`, `objectiveIds`, `sourceRefs`, `needsReview`, and `reviewNotes`. It maps `summary` to one canonical `summaries` item, `keyTerms` to `terms`, `workedExamples` to `examples`, `commonMistakes` to `mistakes`, `examTips` to `examTips`, and each `recap` string to a canonical `recaps` item. Every compiled item inherits or narrows the lesson's non-empty `sourceRefs`; it never loses evidence. `learningObjectives[].id` and `objectiveIds` must contain the same IDs in the same order.
